...
class addkey(commands.Cog):
  ...
  async def addkey(self, ctx, key, *value:str):
    if await stcheck(ctx) is True:
      if key != "del":
        db[key] = value
        await ctx.reply(embed=discord.Embed(title=f"Set {key} to {value}",color=discord.Colour.random()), delete_after=db["del"])
      else:
  ...
  async def delafter(self, ctx, delf:int=5):
    if await stcheck(ctx) is True:
      db["del"] = delf
      await ctx.reply(embed=discord.Embed(title=f"Set the del_after to {delf}.",color=discord.Colour.random()), delete_after=db["del"])
    await ctx.message.delete()
def setup(bot):
