...
class staffRole(commands.Cog, name="Set staffrole"):
  ...
  async def staffRole(self, ctx, arg, rolement: discord.Role=712458500940496936):
    if arg == "set":
      if ctx.author.id == 562175882412687361 or ctx.author.id == 876055467678375998 or ctx.author.id == ctx.guild.owner.id or ctx.author.guild_permissions.administrator is True:
      ...
      if ctx.author.id == 562175882412687361 or ctx.author.id == ctx.guild.owner.id or ctx.author.guild_permissions.administrator is True:
        db.set("staffRole","")
