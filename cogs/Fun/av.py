...
class getAv(commands.Cog):
    """Generate and show avatar commands.
    """
    ...
    async def avatar(self, ctx, member: discord.Member = None):
        if member is None:
            await ctx.reply(
                embed=discord.Embed(
                    title="You need to mention a member!", color=0xEA5852
                ),
                delete_after=db["del"],
            )
        else:
    ...
    @commands.command(
        help="Generates a random default avatar for the user with or without a provided seed."
    )
