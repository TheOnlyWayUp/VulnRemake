...
async def returnLevel(ign=None):
    data = requests.get(
        f"https://api.hypixel.net/player?key={API_KEY}&uuid={await returnUUID(ign)}"
    ).json()["player"]["networkExp"]
    if data is None:
        return 404
    return int(
        math.floor(
            1
            + REVERSE_PQ_PREFIX
            + math.sqrt(REVERSE_CONST + GROWTH_DIVIDES_2 * int(data))
        )
    )
...
async def returnGuild(ign=None):
    try:
        ...
        return requests.get(
            f"https://api.hypixel.net/guild?key={API_KEY}&player={uuid}"
        ).json()["guild"]["name"]
    except:
        return "N/A"
